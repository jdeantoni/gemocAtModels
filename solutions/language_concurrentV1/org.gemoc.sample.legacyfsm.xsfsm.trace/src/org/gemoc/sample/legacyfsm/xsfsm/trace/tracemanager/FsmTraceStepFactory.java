	...
	public fr.inria.diverse.trace.commons.model.trace.Step createStep(
		...
		if (mse.getAction().getName().equalsIgnoreCase("initialize")
				&& (ec.getClassifierID() == org.gemoc.sample.legacyfsm.xsfsm.xsfsm.fsm.FsmPackage.eINSTANCE
						.getFSMSystem().getClassifierID()))

		{
			step = fsmTrace.Steps.StepsFactory.eINSTANCE.createFsm_FSMSystem_Initialize();
		} else
		...
		{
			step = fsmTrace.Steps.StepsFactory.eINSTANCE.createFsm_StateMachine_Run();
		} else
