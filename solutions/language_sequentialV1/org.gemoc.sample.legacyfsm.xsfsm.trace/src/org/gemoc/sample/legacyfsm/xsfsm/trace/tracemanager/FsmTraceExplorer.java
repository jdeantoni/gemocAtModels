...
public class FsmTraceExplorer implements ITraceExplorer {
	...
	private List<List<? extends fsmTrace.States.Value>> getAllValueTraces() {
		...
		for (fsmTrace.States.fsm.TracedBuffer tracedObject : traceRoot.getFsm_tracedBuffers()) {
	...
	private void goTo(EObject eObject) {
		if (eObject instanceof fsmTrace.States.State) {
			...
			for (fsmTrace.States.Buffer_currentValues_Value value : stateToGo.getBuffer_currentValues_Values()) {
				if (value.getParent() instanceof fsmTrace.States.fsm.TracedBuffer) {
					fsmTrace.States.fsm.TracedBuffer parent_cast = (fsmTrace.States.fsm.TracedBuffer) value.getParent();
					org.gemoc.sample.legacyfsm.xsfsm.xsfsm.fsm.Buffer originalObject = (org.gemoc.sample.legacyfsm.xsfsm.xsfsm.fsm.Buffer) parent_cast
							.getOriginalObject();
